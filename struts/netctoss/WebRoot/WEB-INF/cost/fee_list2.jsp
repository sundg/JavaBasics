            ...
            <form action="" method="">
                ...
                <div id="data">            
                    <table id="datalist">
                        ...
                        <tr>
                            <td>${cost.id}</td>
                            <td><a href="detail.action?id=${cost.id}&page=${page}">${cost.name}</a></td>
                            <td>${cost.base_duration}</td>
                            <td>${cost.base_cost}</td>
                            <td>${cost.unit_cost}</td>
                            <td>
                            	<!--<fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${cost.createtime}"/>-->
                            	${cost.ctime}
                            </td>
                            <td>${cost.stime}</td>
                            <td>
                            ...
                            <td>        
                            	<c:choose>
                            		<c:when test="${cost.status==0}">
                            			<input type="button" value="暂停" class="btn_pause" onclick="pauseFee(${cost.id},${page});" />
                            		</c:when>
                            		<c:when test="${cost.status==1}">
                            			<input type="button" value="启用" class="btn_start" onclick="startFee(${cost.id},${page});" />
                            		</c:when>
                            	</c:choose>                        
                                
                                <input type="button" value="修改" class="btn_modify" onclick="location.href='modify.action?id=${cost.id}&page=${page}'" />
                ...
                <div id="pages">
                	...
                	<c:choose>
                		<c:when test="${page==1}">
                			<a href="feelist.action?page=1">上一页</a>
                		</c:when>
                		<c:otherwise>
		        	        <a href="feelist.action?page=${page-1}">上一页</a>
                		</c:otherwise>
                	</c:choose>
        	        <c:forEach begin="1" end="${totalPages}" var="p">
        	        <!-- 判断是否是当前页，而page的值就是当前页 -->
        	        <c:choose>
        	        	<c:when test="${page==p}">	
                    		<a href="feelist.action?page=${p}" class="current_page">${p}</a>
        	       		</c:when>
        	       		<c:otherwise>
        	       			<a href="feelist.action?page=${p}">${p}</a>
        	       		</c:otherwise>
        	        </c:choose>
                    </c:forEach>
                    <!-- 判断是否是最后一页 -->
                	<c:choose>
                		<c:when test="${page==totalPages}">
                			<a href="feelist.action?page=${totalPages}">下一页</a>
                		</c:when>
                		<c:otherwise>
